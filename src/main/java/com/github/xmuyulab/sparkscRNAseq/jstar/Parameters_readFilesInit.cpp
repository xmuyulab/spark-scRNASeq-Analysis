...
{//initialize read files - but do not open yet
    ...
    if (readFilesType.at(0)=="Fastx") {
        *inOut->logStdOut << "readFilesType.at(0) == \"Fastx\"\n" << flush;
        readFilesTypeN=1;
    ...
    *inOut->logStdOut << "readFilesPrefix = " + readFilesPrefix + "\n" << flush;

    readFilesPrefixFinal=(readFilesPrefix=="-" ? "" : readFilesPrefix);

    *inOut->logStdOut << "readFilesManifest[0] = " + readFilesManifest[0] + "\n" << flush;
    *inOut->logStdOut << "readFilesIn.size()" << readFilesIn.size() << "\n" << flush;
    
    if (readFilesManifest[0]=="-") {//no manifest, file names in readFilesIn, 我们修改的代码readFilesManifest[0] == "-"
        ...
        for (uint32 imate=0; imate<readFilesNames.size(); imate++) {
            ...
            //*inOut->logStdOut << "readFilesNames[imate]" + readFilesNames[imate] + "\n" << flush;
        ...
        *inOut->logStdOut << "readFilesNames[0]" << readFilesNames[0][0] << "\n" << flush;
