...
void Parameters::openReadsFiles() {
    ...
    inOut->logMain<<"readFilesIn.at(0) = "<<readFilesIn.at(0)<<"readFilesCommandString = .\n"<<readFilesCommandString<<" .\n";
    ...
    if (readFilesCommandString=="") {//read from file
        for (uint ii=0;ii<readFilesIn.size();ii++) {//open readIn files
         ...
         inOut->logMain<<"readFilesIn.size() = "<<readFilesIn.size();
         for (uint imate=0;imate<readFilesIn.size();imate++) {//open readIn files
            ostringstream sysCom;
            inOut->logMain<<"outFiletmp "<<imate<<" = "<<outFileTmp<<" .\n";
            sysCom << outFileTmp <<"tmp.fifo.read"<<imate+1;
            inOut->logMain<<"sysCom.str() "<<imate<<" = "<<sysCom.str()<<" .\n";
            readFilesInTmp.push_back(sysCom.str());
            inOut->logMain<<"readFilesInTmp.at("<<imate<<") = "<<readFilesInTmp.at(imate)<<".\n";
	    remove(readFilesInTmp.at(imate).c_str());
	    inOut->logMain<<"readFilesInTmp.at("<<imate<<") = "<<readFilesInTmp.at(imate)<<".\n";
	    inOut->logMain<<"S_IRUSR = "<<S_IRUSR<<".\n";
	    inOut->logMain<<"S_IWUSR = "<<S_IWUSR<<".\n";
	    ...
	    inOut->logMain<<"sysShell = "<<sysShell<<".\n";
		...
		inOut->logMain<<"readFilesInString = "<<readFilesInString<<"\n";
                string rfName = (readFilesPrefix=="-" ? "" : readFilesPrefix) + readFilesInString.substr(0, pos);
                inOut->logMain<<"rfName = "<<rfName<<".\n";
...
//             system((("\""+readsCommandFileName.at(imate)+"\"") + " & ").c_str());
    ...
    readFilesIndex=0;
    inOut->logMain << "readFilesTypeN = "<<readFilesTypeN<<" .\n";
